...
def partition_with_metis(adj_list, features, labels, idx_train, idx_val, idx_test, num_parts, add_1hop=False):
    base_result = [(0, adj_list, features, labels, idx_train, idx_val, idx_test, torch.arange(features.size(0)))]
    ...
    for part_idx in range(num_parts):
        nodes_orig = torch.nonzero(cluster == part_idx, as_tuple=False).view(-1)
        if nodes_orig.numel() == 0:
            continue
        ...
        if add_1hop:
            ...
            mask[nodes_orig] = True
            neighbor_mask = mask[row]
            neighbors = col[neighbor_mask]
            nodes_all = torch.cat([nodes_orig, neighbors]).unique()
        else:
            nodes_all = nodes_orig

        part = extract_subgraph_by_nodes(adj_list, features, labels, idx_train, idx_val, idx_test, nodes_all)
        partitions.append((part_idx, *part, nodes_all))
...
if __name__ == '__main__':
    ...
    data_list = ['ogbn-arxiv']  # 'cora','citeseer',arxiv
    path_list = ['new']  # 'new_pair','class_confusion','feature_based','new'

    for data in data_list:
        label_noise_list = [0.3]
        graph_noise_list = [0.3]
        feature_noise_list = [0.3]
            ...
            for graph_noise in graph_noise_list:
                for feature_noise in feature_noise_list:
                    for path in path_list:
                        ...
                        for part_id, adj_part, feat_part, label_part, idx_train_part, idx_val_part, idx_test_part, nodes_all in partitions:
                            ...
                            with open(os.path.join(root_dir, f'{data}_nodes_all{part_suffix}.pkl'), 'wb') as f:
