...
class GCN(nn.Module):
    def __init__(self,
        ...
        if num_layers == 1:
            ...
            # self.gcn_layers.append(GCNConv(
            #     in_dim, num_hidden,bias=True, residual=residual, norm=norm, activation=create_activation(activation)))
            self.gcn_layers.append(GCNConv(
                in_dim, num_hidden,bias=True))
            # hidden layers
            for l in range(1, num_layers - 1):
                # due to multi-head, the in_dim = num_hidden * num_heads
                self.gcn_layers.append(GCNConv(
                num_hidden, num_hidden,bias=True))
            # output projection
            self.gcn_layers.append(GCNConv(
                num_hidden, out_dim,bias=True))
    ...
    def forward(self, g,weights, inputs, return_hidden=False):
        h = inputs
        hidden_list = []
        for l in range(self.num_layers):
            h = F.dropout(h, p=self.dropout, training=self.training)
            h = self.gcn_layers[l](x=h,edge_index=g,edge_weight=weights)
