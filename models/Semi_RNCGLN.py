...
def Ncontrast(x_dis, adj_label, tau = 1, train_index_sort=None):# lgsi
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x):
        x2 = self.norm_1(x)
        x_pre = self.effectattn(x2)#Multi-Head Self-Attention Module
        x = x + x_pre
        x2 = self.norm_2(x)
...
class RNCGLN_model(nn.Module):
    def __init__(self, arg):
        ...
        self.MLPlast = nn.Linear(arg.trans_dim,arg.output_size)
...
class RNCGLN(embedder_single):
    def __init__(self, args,data,g,l):
        embedder_single.__init__(self, args,data,g,l)
        self.args = args
        with open('data\{}\g_{}_l_{}\\nclass.pkl'.format(data,g,l,data), 'rb') as f:
            self.args.nclasses  = pickle.load(f, encoding='latin1')

        self.model = RNCGLN_model(self.args).to(self.args.device)

    def training(self,data,g,l):


        #features = self.features.to(self.args.device)
        with open('data\{}\g_{}_l_{}\{}_features.pkl'.format(data,g,l,data), 'rb') as f:
        ...
        # with open('D:\Andrea\code\RNCGLN-master\data\{}\g_{}_l_{}\{}_adj.pkl'.format(data,g,l,data), 'rb') as f:
        #     graph_org_torch = pickle.load(f, encoding='latin1')
        #graph_org_torch = graph_org_torch.toarray()

        print('mulu'+os.getcwd())
        # here
        graph_noise = scipy.sparse.load_npz('data\{}\g_{}_l_{}\{}_mod_adj_add_{}.npz'.format(data,g,l,data,g))
        graph_noise=graph_noise.toarray()
        graph_noise=torch.tensor(graph_noise,dtype=torch.float64,device='cuda:0')
        graph_org_torch=graph_noise
        ...
        with open('data\{}\g_{}_l_{}\{}_train_index.pkl'.format(data,g,l,data), 'rb') as f:
            self.idx_train  = pickle.load(f, encoding='latin1') #
        self.idx_train = np.array(self.idx_train)
        self.idx_train = self.idx_train.reshape(-1)
        self.idx_train = torch.tensor(self.idx_train, dtype=torch.int64, device='cuda:0')
        with open('data\{}\g_{}_l_{}\{}_val_index.pkl'.format(data, g, l, data),
                  'rb') as f:
            self.idx_val = pickle.load(f, encoding='latin1')  #
        self.idx_val = np.array(self.idx_val)
        self.idx_val = self.idx_val.reshape(-1)
        self.idx_val = torch.tensor(self.idx_val, dtype=torch.int64, device='cuda:0')
        print(self.idx_val)
        with open('data\{}\g_{}_l_{}\{}_test_index.pkl'.format(data, g, l, data),
                  'rb') as f:
            self.idx_test = pickle.load(f, encoding='latin1')  #
        self.idx_test = np.array(self.idx_test)
        self.idx_test = self.idx_test.reshape(-1)
        self.idx_test=torch.tensor(self.idx_test,dtype=torch.int64,device='cuda:0')
        #self.idx_test = torch.where(self.idx_test == 1)[0]


        with open('data\{}\g_{}_l_{}\{}_all_label.pkl'.format(data,g,l,data), 'rb') as f:
            self.labels = pickle.load(f, encoding='latin1')
        self.labels=self.labels.int()
        print(self.labels.dtype)
        self.labels=torch.tensor(self.labels,device='cuda:0')
        #self.labels = torch.from_numpy(self.labels).to(self.args.device)
        print(self.labels)
        self.label_ori = self.labels.clone()

        #train_lbls = self.label_ori[self.idx_train]
        with open('data\{}\g_{}_l_{}\{}_train_label_new_{}.pkl'.format(data,g,l,data,l), 'rb') as f:
            noise_y = pickle.load(f, encoding='latin1')#
        noise_y=np.array(noise_y)
        noise_y=noise_y.reshape(-1).astype(int)
        noise_y = torch.tensor(noise_y,device='cuda:0')


        self.labels[self.idx_train] = noise_y
        print(self.labels)
        ...
        self.labels_oneHot = ones.index_select(torch.tensor(0,device='cuda:0'),torch.tensor(self.labels,device='cuda:0'))
        ...
        adj_label = get_A_r(graph_org_torch,self.args.order)#生成多跳图
        ...
        for epoch in range(self.args.nb_epochs):
            ...
            loss_cla = F.cross_entropy(embeds_tra[self.idx_train], self.labels_oneHot[self.idx_train])#llsi
            max_va_pos = embeds_tra.max(1)[0]
            max_va_pos_index = max_va_pos >= PP#选出置信度大于pp的点
            # max_va_pos_index = max_va_pos >= 0
            loss_Ncontrast = Ncontrast(x_dis, adj_label, tau=self.args.tau, train_index_sort = max_va_pos_index)#lgsi
            ...
            if epoch % 5 == 0 and epoch != 0:
                ...
                self.model.eval()
                torch.save(self.model, './result/{}_g{}_l{}.pth'.format(data,g,l))
                ...
                # test_lbls=test_lbls.toarray()
                #embeds[self.idx_test].cpu().numpy()
                # embeds[self.idx_test] = embeds[self.idx_test].toarray()
                AP_val = average_precision_score(val_lbls.cpu().detach().numpy(),
                                                  embeds[self.idx_val].cpu().detach().numpy())

                AP_test=average_precision_score(test_lbls.cpu().detach().numpy(), embeds[self.idx_test].cpu().detach().numpy())


                # 计算F1分数
                label_oh_val = []
                for i in embeds[self.idx_val]:
                    label_oh_val.append(np.argmax(i.cpu().detach().numpy()))
                label_oh_val = np.array(label_oh_val)
                f1_val = f1_score(val_lbls.cpu().detach().numpy(), label_oh_val, average='micro')
                label_oh_test=[]
                for i in embeds[self.idx_test]:
                    label_oh_test.append(np.argmax(i.cpu().detach().numpy()))
                label_oh_test=np.array(label_oh_test)
                f1_test = f1_score(test_lbls.cpu().detach().numpy(), label_oh_test,average='micro')
                ...
                if val_acc > best:
                    ...
                    # if better_num > self.args.warmup_num and self.args.IsLabNoise and training_turn_ind:
                        pre_value_max, pre_index_max = embeds.max(1) #选取最大的置信度
                        self.labels_oneHot = embeds.detach().clone() #得到分类
                        Y_zero = torch.zeros_like(self.labels_oneHot)
                        Y_zero.scatter_(1,pre_index_max.unsqueeze(1),1)
                        self.labels_oneHot[pre_value_max >= self.args.P_sel_onehot] = Y_zero[pre_value_max >= self.args.P_sel_onehot]#取置信度大于0.95的为1
                        pre_ind_min = pre_value_max >=self.args.P_sel
                        self.idx_train = pre_ind_min.float() +train_all_pos.float() == 2#取置信度大于0.9的点保留在训练集
                    ...
                    if epoch > self.args.warmup_num and self.args.IsGraNoise and better_inex:
                        ## update GRAPH
                        x_dis_mid = x_dis.detach().clone()#获取所有点之间的distance 2708*2708

                        x_dis_mid = x_dis_mid * adj_label
                        val_, pos_ = torch.topk(x_dis_mid, int(x_dis_mid.size(0) * (1 - self.args.P_gra_sel)))#选取前270个最大的数
                        adj_label = torch.where(x_dis_mid > val_[:, -1].unsqueeze(1), x_dis_mid, 0)#更新多跳图
        ...
        print("\n [Classification] ACC: {:.4f} | stop_epoch: {:}| training_time: {:.4f} |AP:{:.4f}|F1:{:.4f}\n".format(
            output_acc, stop_epoch, training_time,output_AP,output_f1))


        return output_acc, training_time, stop_epoch,output_AP,output_f1
