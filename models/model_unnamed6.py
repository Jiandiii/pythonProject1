...
class unnamed_model6(nn.Module):
    def __init__(self, args, input_size, hidden_size, output_size, num_classes, num_domains=1):
        ...
        super(unnamed_model6, self).__init__()
        self.args = args
        ...
        # 移除低效的字典存储
        self.ema_decay = 0.9        # EMA 平滑系数
        self.align_weight = 2.0    # 适当调高权重，因为有了置信度过滤，对齐更安全了
        self.conf_threshold = 0.5   # 降低门槛，让更多节点参与对齐
    ...
    def fit(self, all_partitions):
        ...
        for part in all_partitions:
            if part['nodes_all'] is not None:
                max_node_id = max(max_node_id, part['nodes_all'].max().item())
        
        total_nodes = max_node_id + 1
        self.global_embeddings_tensor = torch.zeros((total_nodes, args.hidden_size), device=self.device)
        self.global_logits_tensor = torch.zeros((total_nodes, first_part['num_classes']), device=self.device)
        self.global_mask = torch.zeros(total_nodes, dtype=torch.bool, device=self.device)
        
        # 新增：全局标签池
        self.global_train_labels = torch.full((total_nodes,), -1, dtype=torch.long, device=self.device)
        for part in all_partitions:
            if part['nodes_all'] is not None:
                # 确保索引操作在同一设备上进行
                p_nodes_gpu = part['nodes_all'].to(self.device)
                p_train_idx_cpu = part['train_index'].cpu()
                p_train_labels_gpu = part['labels_noise'][p_train_idx_cpu].to(self.device)
                
                # 在 GPU 上更新全局标签池
                p_train_idx_gpu = p_train_idx_cpu.to(self.device)
                self.global_train_labels[p_nodes_gpu[p_train_idx_gpu]] = p_train_labels_gpu
        
        print(f"Global Label Pool initialized: {torch.sum(self.global_train_labels != -1).item()} labeled nodes found.")
        ...
        for epoch in range(args.epochs):
            ...
            # 联合训练：遍历所有子图
            import random
            indices = list(range(len(all_partitions)))
            random.shuffle(indices)
            
            for idx in indices:
                part_data = all_partitions[idx]
                domain_id = idx # 使用原始索引作为 domain_id
                part_data['part_id'] = domain_id # 确保 part_id 被记录
                ...
                nodes_all = part_data.get('nodes_all', None)
                if nodes_all is not None:
                    nodes_all = nodes_all.to(self.device)
                ...
                if 'soft_label1' in part_data:
                    self.args.soft_label1 = part_data['soft_label1'].to(self.device)
                    self.args.soft_label2 = part_data['soft_label2'].to(self.device)
                    self.args.soft_label3 = part_data['soft_label3'].to(self.device)
                    self.args.soft_label4 = part_data['soft_label4'].to(self.device)
                ...
                (train_acc, val_acc, test_acc), t, loss_val, pred_weights, pred_edges, pred_feats = self.train(epoch, features, edge_index, edge_weight, 
                                                                        ...
                                                                        domain_id=domain_id, nodes_all=nodes_all)
    ...
    def train(self, epoch, features, edge_index, edge_weight, train_mask, val_mask, test_mask, domain_id=0, nodes_all=None):
        ...
        # 锚点对齐训练
        loss_align = torch.tensor(0.0).to(self.device)
        loss_extra_sup = torch.tensor(0.0).to(self.device)
        
        if nodes_all is not None:
            # 1. 全局标签共享监督 (Anchor Label Sharing)
            # 即使当前节点在当前子图中不是训练集，但如果在其他子图中是训练集，我们也利用它的标签
            global_labels = self.global_train_labels[nodes_all]
            extra_train_mask = (global_labels != -1)
            if extra_train_mask.any():
                loss_extra_sup = F.cross_entropy(output_z_y1[extra_train_mask], global_labels[extra_train_mask])

            # 2. 锚点一致性对齐 (原有逻辑)
            mask = self.global_mask[nodes_all]
            if epoch >= 20:
                if mask.any():
                    seen_indices = torch.where(mask)[0]
                    target_nodes = nodes_all[seen_indices]
                    
                    # 改用 Top-K 百分比过滤 (取前 20% 最自信的锚点)
                    with torch.no_grad():
                        target_logits = self.global_logits_tensor[target_nodes]
                        conf = F.softmax(target_logits, dim=1).max(1)[0]
                        k = max(1, int(len(seen_indices) * 0.2))
                        _, topk_indices = torch.topk(conf, k)
                        valid_indices = seen_indices[topk_indices]
                        valid_target_nodes = target_nodes[topk_indices]
                    
                    if len(valid_indices) > 0:
                        if epoch % 20 == 0 and domain_id == 0:
                            print(f"--- Alignment Stats (Epoch {epoch+1}) ---")
                            print(f"Total Anchors: {len(seen_indices)}, Top-K Aligned: {len(valid_indices)}")
                        
                        norm_rep = F.normalize(representations1[valid_indices], p=2, dim=1)
                        norm_tar = F.normalize(self.global_embeddings_tensor[valid_target_nodes].detach(), p=2, dim=1)
                        loss_feat_align = F.mse_loss(norm_rep, norm_tar)
                        
                        log_p = F.log_softmax(output_z_y1[valid_indices], dim=1)
                        target_p = F.softmax(self.global_logits_tensor[valid_target_nodes].detach(), dim=1)
                        loss_logit_align = F.kl_div(log_p, target_p, reduction='batchmean')
                        
                        dynamic_weight = min(self.align_weight, self.align_weight * (epoch / 100.0))
                        loss_align = dynamic_weight * (loss_feat_align + loss_logit_align)
            ...
            # 更新已存在的
            if mask.any():
                existing_indices = torch.where(mask)[0]
                target_nodes = nodes_all[existing_indices]
                self.global_embeddings_tensor[target_nodes] = self.ema_decay * self.global_embeddings_tensor[target_nodes] + (1 - self.ema_decay) * representations1[existing_indices].detach()
                self.global_logits_tensor[target_nodes] = self.ema_decay * self.global_logits_tensor[target_nodes] + (1 - self.ema_decay) * output_z_y1[existing_indices].detach()
            
            # 插入新节点
            if new_mask.any():
                new_indices = torch.where(new_mask)[0]
                target_nodes = nodes_all[new_indices]
                self.global_embeddings_tensor[target_nodes] = representations1[new_indices].detach()
                self.global_logits_tensor[target_nodes] = output_z_y1[new_indices].detach()
                self.global_mask[target_nodes] = True
        ...
        if self.args.teacher_student == 'student':
            if self.args.ntea == 4:
                loss_all1 = loss1 + 0.03 * rec_loss_s1 + rec_loss_f1 + (kld1 + kld2 + kld3 + kld4) * self.args.lambda_kld + loss_align
            elif self.args.ntea == 3:
                loss_all1 = loss1 + 0.03 * rec_loss_s1 + rec_loss_f1 + (kld1 + kld2 + kld3) * self.args.lambda_kld + loss_align
            elif self.args.ntea == 2:
                loss_all1 = loss1 + 0.03 * rec_loss_s1 + rec_loss_f1 + (kld1 + kld2) * self.args.lambda_kld + loss_align
            elif self.args.ntea == 1:
                loss_all1 = loss1 + 0.03 * rec_loss_s1 + rec_loss_f1 + (kld1) * self.args.lambda_kld + loss_align
        else:
            loss_all1 = loss1 + 0.03 * rec_loss_s1 + rec_loss_f1 + loss_align + loss_extra_sup
...
class q_phi1(nn.Module):
    ...
    def get_estimated_weigths(self, edge_index, representations):
        ...
        batch_size = 1000000  # 每批处理 100 万条边
        weights = []
        
        for i in range(0, num_edges, batch_size):
            end = min(i + batch_size, num_edges)
            batch_edges = edge_index[:, i:end]
            x0 = representations[batch_edges[0]]
            x1 = representations[batch_edges[1]]
            batch_weights = torch.sum(torch.mul(x0, x1), dim=1)
            weights.append(batch_weights)
            
        output = torch.cat(weights)
