...
def load_noisydata(noise_type, dataset, graph_noise, label_noise, feature_noise, part_id=None):
    ...
    try:
        with open(datapath + '{}_nodes_all{}.pkl'.format(dataset, part_suffix), 'rb') as f:
            nodes_all = pickle.load(f, encoding='latin1')
    except FileNotFoundError:
        nodes_all = None

    return train_idx, val_idx, test_idx, train_label, val_label, test_label, features, adj, true_train_label, num_classes, labels, nodes_all
